...
public class SimplifiedOdometryRobot {
    ...
    private static final double STRAFE_ACCEL        = 3.0;     // Acceleration limit.  Percent Power change per second.  1.0 = 0-100% power in 1 sec.
    ...
    private ElapsedTime timeOutTimer = new ElapsedTime();  // User for any motion requiring a hold time or timeout.
    ...
    public void initialize(boolean showTelemetry)
    {
        ...
        leftFrontDrive  = setupDriveMotor("drv2", DcMotor.Direction.FORWARD);
        rightFrontDrive = setupDriveMotor("drv1", DcMotor.Direction.REVERSE);
        leftBackDrive  = setupDriveMotor( "drv3", DcMotor.Direction.FORWARD);
        rightBackDrive = setupDriveMotor( "drv4",DcMotor.Direction.FORWARD);
        imu = myOpMode.hardwareMap.get(IMU.class, "imu");

        //  Connect to the encoder channels using the name of that channel.
        driveEncoder = myOpMode.hardwareMap.get(DcMotor.class, "drv2");
        strafeEncoder = myOpMode.hardwareMap.get(DcMotor.class, "drv4");
    ...
    public void drive(double distanceInches, double power, double holdTime, double timeOut) {
        ...
        while (myOpMode.opModeIsActive() && readSensors() && timeOutTimer.time() < timeOut){
