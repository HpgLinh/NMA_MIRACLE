...
@TeleOp(name="p0xa_dinh", group = "shiningdanyang")
public class p0xa_dinh extends LinearOpMode
{
    double P = 50;
    double I = 0.6;
    double D = 0.001;
    ...
    @Override public void runOpMode()
    {
        ...
        // ==== start =====
        srvTurn(srvTurret0, -0.15);
        srvTurn(srvTurret1, -0.15);//left
        srvOval.setPosition(0.06);
        shooter.setVelocity(900);
        shootstraff.setVelocity(900);
        srvLoad.setPosition(0);
        motorIntake.setPower(0.5);

        while(opModeIsActive() && robot.strafeDistance > -30) moveDrivetrain(-1.0, 0.0, 0.0);
        waitTimer.reset();
        srvLoad.setPosition(0.35);
        while(waitTimer.seconds() <= 2) moveDrivetrain(0.0, 0.0, 0.0);
        srvLoad.setPosition(0);
        motorIntake.setPower(1);
        srvTurn(srvTurret0, -0.18);
        srvTurn(srvTurret1, -0.18);//
        
        while(opModeIsActive() && robot.strafeDistance > -62) moveDrivetrain(-0.8, 0.0, 0.0);   
        while(opModeIsActive() && robot.driveDistance < 36) moveDrivetrain(0, 0.8, 0);
        motorIntake.setPower(0.5);
        while(opModeIsActive() && robot.driveDistance > 15) moveDrivetrain(0.2, -0.8, 0);
        ...
        while(opModeIsActive() && robot.strafeDistance > -95) moveDrivetrain(-0.8, 0, 0);
        while(opModeIsActive() && robot.driveDistance < 35) moveDrivetrain(0, 0.8, 0);
        while(opModeIsActive() && robot.driveDistance > 9) moveDrivetrain(0.7, -0.8, 0);
        
        waitTimer.reset();
        srvLoad.setPosition(0.35);
        while(waitTimer.seconds() <= 2) moveDrivetrain(0.0, 0.0, 0.0);
           shooter.setVelocity(1120);
        shootstraff.setVelocity(1120);
        srvTurn(srvTurret0, -0.25);
        srvTurn(srvTurret1, -0.25);
        srvLoad.setPosition(0);
        motorIntake.setPower(1);
        
        while(opModeIsActive() && robot.strafeDistance > -125) moveDrivetrain(-1, 0, 0);
        while(opModeIsActive() && robot.driveDistance < 38) moveDrivetrain(0, 0.8, 0);
        motorIntake.setPower(0.6);
        while(opModeIsActive() && robot.driveDistance > -10) moveDrivetrain(-0.3, -0.9, 0);
        srvLoad.setPosition(0.35);
