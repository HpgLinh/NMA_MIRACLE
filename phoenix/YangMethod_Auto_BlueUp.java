...
@Autonomous(name = "Dương's Method Auto Blue Up", group = "Drive")
public class YangMethod_Auto_BlueUp extends LinearOpMode {
    ...
    void active_shoot(double ovalPos, int shootSpeed) {
        ...
        while (waitTimer.seconds() <= 1.2 && opModeIsActive()) {
            moveDrvTrain(0, 0, 0);
        }
        srvLoad.setPosition(0.12);
        intakeDrive.setVelocity(7000);
    }
    ...
    public void moveDrvTrain(double x, double y, double z) {
        ...
        double kpStill = 0.7;      // bạn có thể thử 0.25 -> 0.5
        double kdStill = 0.03;
        ...
        turret.setPosition(Math.toDegrees(angle_unit) - 0.5);
    ...
    public void step0() {
        ...
        while (rawX > -8000 && opModeIsActive()) {
            moveDrvTrain(-1, -0.8, 0);
        }

        active_shoot(0.06, 1600);
    }
    ...
    public void step1() {
        ...
        // --- Vào Intake hàng 2 ---
        while (rawY < 10000 && opModeIsActive()) {
        ...
        while (rawY > -1000 && opModeIsActive()) {
            moveDrvTrain(0.8, -1, 0);
        }

        // --- Di chuyển tới vị trí bắn ---
        while (rawX < -28000 && opModeIsActive()) {
            moveDrvTrain(1, 0, 0);
        }
    ...
    public void step2() {
        ...
        // --- Đi ngang về hàng 2 ---
        while (rawX > -32200 && opModeIsActive()) {
            moveDrvTrain(-1, 0, 0);
        }
        ...
        // --- Tiến vào gạt cần ---
        while (Math.abs(Math.toDegrees(botHeading)) < 30 && opModeIsActive()) {
            moveDrvTrain(0, 1, -0.4);
        }
        while (rawY < 5500 && opModeIsActive()) {
            moveDrvTrain(0, 1, 0);
        }
        while (rawY < 9000 && opModeIsActive()) {
            moveDrvTrain(0, 0.5, 0);
        }

        time0.reset();
        while (time0.seconds() < 1.5 && opModeIsActive()) {
            moveDrvTrain(0, 0.1, 0);
        }
        
        // --- trượt ngang để đón bóng ---
        time0.reset();
        while (time0.seconds() <= 0.6 && opModeIsActive())
        {
            moveDrvTrain(-0.4, 0.3, 0);
        }
        
        time0.reset();
        while (time0.seconds() <= 0.8 && opModeIsActive()) {
            moveDrvTrain(0, 0, 0);
        }
        ...
        // --- lui ra ---
        time0.reset();
        while (time0.seconds() <= 0.3 && opModeIsActive()) {
            moveDrvTrain(0.6, -1, 0);
        ...
        while (rawY > 3000 && opModeIsActive()) {
            moveDrvTrain(0, -1, 0);
        }
        ...
        active_shoot(0.06, 1800);
    }

    public void step3() {
        // --- Giai đoạn 3: Di chuyển tới hàng 1 và intake ---
        while (rawX > -25500 && opModeIsActive()) {
            moveDrvTrain(-1, 0.2, 0);
        }
        while (rawY < 3000 && opModeIsActive()) {
            moveDrvTrain(0, 0.8, 0);
        }

        while (rawY > -4000 && opModeIsActive()) {
            moveDrvTrain(0, -1, 0);
        }

        active_shoot(0.07, 1850);
    }

    public void step4() {
        // --- Giai đoạn 4: Di chuyển tới hàng 3 và intake ---
        while (rawX > -47000 && opModeIsActive()) {
            moveDrvTrain(-1, 0, 0);
        }

        while (rawY < 7300 && opModeIsActive()) {
            moveDrvTrain(0, 0.8, 0);
        }

        while (rawY > 0000 && opModeIsActive()) {
            moveDrvTrain(0, -1, 0);
        }
    ...
    public void step5() {
        ElapsedTime time0 = new ElapsedTime();
        time0.reset();
        while (time0.seconds() <= 0.9 && opModeIsActive()) {
            moveDrvTrain(-0.4, 0.4, 0);
        }
    ...
    public void runOpMode() throws InterruptedException {
        ...
        intakeDrive.setMode(DcMotorEx.RunMode.RUN_USING_ENCODER);
