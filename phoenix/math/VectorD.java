package org.firstinspires.ftc.teamcode.phoenix.math;

import org.firstinspires.ftc.teamcode.phoenix.geometry.Pose;
import java.util.Vector;

/**
 * This is the VectorD class. This class handles storing information about vectors, which are
 * basically Points but using polar coordinates as the default. The main reason this class exists
 * is because some VectorD math needs to be done in the Follower, and dot products and cross
...
public class VectorD {
    ...
    /**
     * The x component of the VectorD in Cartesian coordinates.
     */
    private double xComponent;
    /**
     * The y component of the VectorD in Cartesian coordinates.
     */
    private double yComponent;

    /**
     * Constructs a new VectorD with zero magnitude and direction.
     */
    public VectorD() {
        setComponents(0, 0);
    }
    ...
    /**
     * Rotates the VectorD by a given angle (in radians).
     *
    ...
    /**
     * This multiplies the current VectorD by a scalar and returns the result as a Vector.
     *
     * @param scalar the scalar multiplying into the Vector.
     * @return returns the scaled Vector.
     */
    public VectorD times(double scalar) {
        return new VectorD(getMagnitude() * scalar, getTheta());
    }

    /**
     * This normalizes this VectorD to be of magnitude 1, unless this VectorD is the zero Vector.
     * In that case, it just returns back the zero VectorD but with a different memory location.
     *
     * @return returns the normalized (or zero) Vector.
     */
    public VectorD normalize() {
        if (getMagnitude() == 0) {
            return new VectorD(0.0, getTheta());
        } else {
            return new VectorD(getMagnitude() / Math.abs(getMagnitude()), getTheta());
        }
    }

    /**
     * This returns a VectorD that is the sum of the this VectorD and the other input Vector.
     *
     * @param other the other Vector.
     * @return returns the sum of the Vectors.
     */
    public VectorD plus(VectorD other) {
        VectorD returnVector = new VectorD();
        returnVector.setOrthogonalComponents(getXComponent() + other.getXComponent(), getYComponent() + other.getYComponent());
        return returnVector;
    }

    /**
     * This subtracts the other VectorD from the current VectorD and returns the result as a Vector.
     * Do note that order matters here.
     *
     * @param other the other Vector.
     * @return returns the second VectorD subtracted from the first Vector.
     */
    ...
    /**
     * This computes the dot product of the current VectorD and the other Vector.
     *
     * @param other the Other Vector.
    ...
    /**
     * This computes the current VectorD crossed with the other Vector, so a cross product.
     * Do note that order matters here.
     *
     * @param other the other Vector.
    ...
    /**
     * This returns a VectorD that is the linear combination of the current VectorD and another vector.
     * @param other the other vector
     * @param scaleThis the coefficient for the current vector
     * @param scaleOther the second coefficient for the other vector
     * @return the resulting vector
     */
    public VectorD linearCombination(VectorD other, double scaleThis, double scaleOther) {
     ...
     *
     * @return a new VectorD with the same magnitude and direction
     */
    public VectorD copy() {
        return new VectorD(this.magnitude, this.theta);
    }
    ...
    /**
     * Transforms the VectorD by multiplying it with a matrix
     *
     * @param matrix the matrix transformation
     * @return the resulting VectorD after applying the transformation
     */
    public VectorD transform(Matrix matrix) {
        double[] multiply = matrix.multiply(new Matrix(new double[][]{{xComponent}, {yComponent}})).getCol(0);
        return new VectorD(multiply[0], multiply[1]);
    }

    /**
     * Projects this VectorD onto another vector.
     *
     * @param other the VectorD to project onto
     * @return a new VectorD that is the projection of this VectorD onto the other vector
     */
    public VectorD projectOnto(VectorD other) {
        if (other.getMagnitude() == 0) return new VectorD();
