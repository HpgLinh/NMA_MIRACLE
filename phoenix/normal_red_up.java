...
@TeleOp(name = "Red Up hpL", group = "Drive")
public class normal_red_up extends LinearOpMode {
    ...
    void active_shoot(double ovalPos, int shootSpeed) {
        ...
        while (waitTimer.seconds() <= 1.5 && opModeIsActive()) {
            moveDrvTrain(0, 0, 0);
        }
    ...
    public void moveDrvTrain(double x, double y, double z) {
        ...
        double angle_unit = Math.atan2(135 - pose.getY(), 138 - pose.getX()) - pose.getHeading();
    ...
    public void step1() {
        ...
        // --- Đi ngang về hàng 2 ---
        while (rawX > -33000 && opModeIsActive()) {
            moveDrvTrain(-1, 0, 0);
        }
        ...
        while (rawY > 8000 && opModeIsActive()) {
            moveDrvTrain(0, -0.4, 0);
        }
        while(rawX < -35000 && opModeIsActive()) {
            moveDrvTrain(0.4, 0, 0);
        }
        while (rawY < 8500 && opModeIsActive()) {
            moveDrvTrain(0, 0.3, 0);
        }
        // --- Di chuyển tới vị trí bắn ---
        
        ElapsedTime time0 = new ElapsedTime();
        time0.reset();
        while(time0.seconds() <= 0.8 && opModeIsActive()) moveDrvTrain(0, 0, 0);

        while(rawY > 800 && opModeIsActive()) {
            moveDrvTrain(0, -0.7, 0);
        }
        while(rawX < -28000 && opModeIsActive()) {
            moveDrvTrain(0.8, 0, 0);
        }
        active_shoot(0.07, 1800);
    }
    ...
    public void step3() {
        ...
        while (rawY < 5000 && opModeIsActive()) {
    ...
    public void step4() {
        ...
        while (rawX > -43000 && opModeIsActive()) {
            moveDrvTrain(-1, 0, 0);
        }

        while (rawY < 7900 && opModeIsActive()) {
            moveDrvTrain(0, 0.8, 0);
        }
