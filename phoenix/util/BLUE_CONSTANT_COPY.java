...
@TeleOp(name = "BLUE_CONSTANT_COPY", group = "Drive")
public class BLUE_CONSTANT_COPY extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            if(toggler2dpaddown.shouldToggle(gamepad2.dpad_down)) headlock_offset += 5;
            if(toggler2dpadup.shouldToggle(gamepad2.dpad_up)) headlock_offset -= 5;
            ...
            if(head_lock) {
                ...
                while (angle_unit <= -Math.PI) angle_unit += 2 * Math.PI;
                turret.setPosition(Math.toDegrees(angle_unit) + headlock_offset);
                power = close_tag ? power_headlock_up : power_headlock_down;
            } else if(!close_tag) {
                if(angleWrap(botHeading) < Math.PI/2 && angleWrap(botHeading) > -Math.PI/2) 
                {
                    turret.setPosition(-66);//heading lúc khởi động
                    srvOval.setPosition(0.12);
                    power = powerForward;
                }
                else
                {
                    turret.setPosition(123);//heading quay ngược về sau (để intake phía sau)
            ...
            boolean rtPress = (gamepad2.right_bumper);
            // if (rtPress && !prevRT) 
            // {
            //     if(angleWrap(botHeading) < Math.PI/2 && angleWrap(botHeading) > -Math.PI/2) 
            //     {
            ...
            if (togglerShootSpeedIncrease.shouldToggle(rtPress)) {
                if(angleWrap(botHeading) < Math.PI/2 && angleWrap(botHeading) > -Math.PI/2 && !head_lock) 
                {
                    powerForward += 50;
                }
                else
            ...
            boolean ltPress = (gamepad2.left_bumper );
            // if (ltPress && !prevLT) 
            // {
            //     if(angleWrap(botHeading) < Math.PI/2 && angleWrap(botHeading) > -Math.PI/2) 
            //     {
            ...
            else if (stateIntake == 1) intakeDrive.setVelocity(loadOpen ? 2800 : 4000);
           ...
           // -----------------------------
            ...
            telemetry.addData("TurretBlue Heading", turret.currentDegree);
            telemetry.addData("TurretBlue offset", turret_offset);
