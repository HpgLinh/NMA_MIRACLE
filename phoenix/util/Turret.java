...
public class Turret {
    ...
    public double Kp = 0.035;  // Tăng lên nếu phản ứng chậm
    ...
    public Turret (HardwareMap hwMap, double offset) {
        ticksoffset = offset;
        turretMotor = hwMap.get(DcMotor.class, "turret_motor");
        turretMotor.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        turretMotor.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
    ...
    public void setPosition(double targetDegree) {
        targetDegree = Math.min(targetDegree, 179.0);
        targetDegree = Math.max(targetDegree, -179.0);
        currentTicks = turretMotor.getCurrentPosition() + ticksoffset;
